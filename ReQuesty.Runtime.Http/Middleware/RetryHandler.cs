...
public class RetryHandler(RetryHandlerOption? retryOption = null) : DelegatingHandler
{
    ...
    {
        get; set;
    } = retryOption ?? new RetryHandlerOption();
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        RetryHandlerOption retryOption = request.GetRequestOption<RetryHandlerOption>() ?? RetryOption;
        ...
        if (request.GetRequestOption<ObservabilityOptions>() is { } obsOptions)
        {
            ...
            activity?.SetTag("requesty.runtime.handler.retry.enable", true);
        }
        else
        {
            activity = null;
    ...
    private async Task<HttpResponseMessage> SendRetryAsync(HttpResponseMessage response, RetryHandlerOption retryOption, CancellationToken cancellationToken, ActivitySource? activitySource)
    {
        ...
        while (retryCount < retryOption.MaxRetry)
        {
            ...
            using Activity? retryActivity = activitySource?.StartActivity($"{nameof(RetryHandler)}_{nameof(SendAsync)} - attempt {retryCount}");
            ...
            Task delay = DelayAsync(response, retryCount, retryOption.Delay, out double delayInSeconds, cancellationToken);
            ...
            HttpRequestMessage request = await originalRequest.CloneAsync(cancellationToken).ConfigureAwait(false);
    ...
    private static async Task<Exception> GetInnerExceptionAsync(HttpResponseMessage response)
    {
        ...
        Dictionary<string, IEnumerable<string>> headersDictionary = [];
