...
public class ChaosHandler : DelegatingHandler, IDisposable
{
    ...
    /// </summary>
    public const string ChaosHandlerTriggeredEventKey = "requesty.runtime.chaos_handler_triggered";
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        ChaosHandlerOption chaosHandlerOptions = request.GetRequestOption<ChaosHandlerOption>() ?? _chaosHandlerOptions;
        Activity? activity;
        if (request.GetRequestOption<ObservabilityOptions>() is { } obsOptions)
        {
            ...
            activity?.SetTag("requesty.runtime.handler.chaos.enable", true);
        }
        else
        {
            activity = null;
        ...
        try
        {
            ...
            if (chaosHandlerOptions.PlannedChaosFactory != null && chaosHandlerOptions.PlannedChaosFactory(request) is HttpResponseMessage plannedResponse)
            {
                ...
                activity?.SetTag("requesty.runtime.handler.chaos.planned", true);
                return plannedResponse;
            }
            else if (_random.Next(100) < chaosHandlerOptions.ChaosPercentLevel)
            {
                HttpResponseMessage chaosResponse = CreateChaosResponse(chaosHandlerOptions.KnownChaos ?? _knownFailures!);
                chaosResponse.RequestMessage = request;
                activity?.AddEvent(new(ChaosHandlerTriggeredEventKey));
                activity?.SetTag("requesty.runtime.handler.chaos.planned", false);
    ...
    {
        int responseIndex = _random.Next(knownFailures.Count);
    ...
    {
        string contentString = JsonSerializer.Serialize(new MainError
        {
            error = new Error
            {
        ...
        HttpResponseMessage throttleResponse = new()
        {
    ...
    public static HttpResponseMessage Create503Response(TimeSpan retry)
    {
        ...
        HttpResponseMessage serverUnavailableResponse = new()
        {
    ...
    public static HttpResponseMessage Create502BadGatewayResponse()
    {
        ...
        HttpResponseMessage badGatewayResponse = new()
        {
    ...
    public static HttpResponseMessage Create500InternalServerErrorResponse()
    {
        ...
        HttpResponseMessage internalServerError = new()
        {
    ...
    public static HttpResponseMessage Create504GatewayTimeoutResponse(TimeSpan retry)
    {
        ...
        HttpResponseMessage gatewayTimeoutResponse = new()
        {
    ...
    public new void Dispose()
    {
        ...
        foreach (HttpResponseMessage response in _knownFailures)
        {
            response.Dispose();
        }
