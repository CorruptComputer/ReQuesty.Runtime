...
public class RedirectHandler(RedirectHandlerOption? redirectOption = null) : DelegatingHandler
{
    /// <summary>
    ...
    {
        get; private set;
    } = redirectOption ?? new RedirectHandlerOption();
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        RedirectHandlerOption redirectOption = request.GetRequestOption<RedirectHandlerOption>() ?? RedirectOption;
        ...
        if (request.GetRequestOption<ObservabilityOptions>() is { } obsOptions)
        {
            ...
            activity?.SetTag("requesty.runtime.handler.redirect.enable", true);
        }
        else
        {
            activity = null;
        ...
        try
        {
            ...
            if (ShouldRedirect(response, redirectOption))
            {
                ...
                {
                    using Activity? redirectActivity = activitySource?.StartActivity($"{nameof(RedirectHandler)}_{nameof(SendAsync)} - redirect {redirectCount}");
                    redirectActivity?.SetTag("requesty.runtime.handler.redirect.count", redirectCount);
                    ...
                    HttpRequestMessage? originalRequest = response.RequestMessage;
                    if (originalRequest == null)
                    {
                        return response;// We can't clone the original request to replay it.
                    }
                    HttpRequestMessage newRequest = await originalRequest.CloneAsync(cancellationToken).ConfigureAwait(false);
                    ...
                    {
                        string? baseAddress = newRequest.RequestUri?.GetComponents(UriComponents.SchemeAndServer | UriComponents.KeepDelimiter, UriFormat.Unescaped);
