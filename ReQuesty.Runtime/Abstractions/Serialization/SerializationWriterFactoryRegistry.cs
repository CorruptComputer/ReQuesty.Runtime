...
public class SerializationWriterFactoryRegistry : ISerializationWriterFactory
{
    ...
    {
        ISerializationWriterFactory factory = GetSerializationWriterFactory(contentType, out string actualContentType);
    ...
    private ISerializationWriterFactory GetSerializationWriterFactory(string contentType, out string actualContentType)
    {
        ...
        string vendorSpecificContentType = contentType.Split(";".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)[0];
        if (ContentTypeAssociatedFactories.TryGetValue(vendorSpecificContentType, out ISerializationWriterFactory? vendorFactory))
        {
            actualContentType = vendorSpecificContentType;
            return vendorFactory;
        }

        string cleanedContentType = ParseNodeFactoryRegistry.contentTypeVendorCleanupRegex.Replace(vendorSpecificContentType, string.Empty);
        if (ContentTypeAssociatedFactories.TryGetValue(cleanedContentType, out ISerializationWriterFactory? factory))
        {
