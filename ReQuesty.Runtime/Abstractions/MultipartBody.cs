...
{
    private readonly Lazy<string> _boundary = new(() => Guid.NewGuid().ToString("N"));
    ...
    public void AddOrReplacePart<T>(string partName, string contentType, T partValue, string? fileName = null)
    {
        ...
        (string partName, string) key = (partName, fileName ?? "");
        Part value = new(partName, partValue, contentType, fileName);
    ...
    {
        T? value = GetPartValue<T>(partName, null);

        if (EqualityComparer<T?>.Default.Equals(value, default))
        {
            foreach ((string, string) key in _parts.Keys)
            {
    ...
    public T? GetPartValue<T>(string partName, string? fileName)
    {
        ...
        if (_parts.TryGetValue((partName, fileName ?? ""), out Part? value))
        {
            if (value == null)
            {
                return default;
    ...
    private readonly Dictionary<ValueTuple<string, string>, Part> _parts = new(new ValueTupleComparer());
    ...
    public void Serialize(ISerializationWriter writer)
    {
        ...
        bool first = true;
        StringBuilder contentDispositionBuilder = new();
        foreach (Part part in _parts.Values)
        {
            try
            {
                ...
                {
                    using ISerializationWriter partWriter = RequestAdapter.SerializationWriterFactory.GetSerializationWriter(part.ContentType);
    ...
    {
        using Stream partContent = partWriter.GetSerializedContent();
