...
public class RequestInformation
{
    ...
    public void Configure<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] T>(Action<RequestConfiguration<T>>? requestConfiguration) where T : class, new()
    {
        ...
        RequestConfiguration<T> requestConfig = new();
    ...
    public Uri URI
    {
        ...
        get
        {
            ...
            else if (PathParameters.TryGetValue(RawUrlKey, out object? rawUrl) &&
            ...
            else
            {
                ...
                foreach (KeyValuePair<string, object> urlTemplateParameter in PathParameters)
                {
                    substitutions.Add(urlTemplateParameter.Key, GetSanitizedValues(urlTemplateParameter.Value));
                }

                foreach (KeyValuePair<string, object> queryStringParameter in QueryParameters)
                {
    ...
    public void AddQueryParameters<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] T>(T source)
    {
        ...
        {
            QueryParameterAttribute? queryParameterAttribute = (QueryParameterAttribute?)null;
            ...
            string name = queryParameterAttribute?.TemplateName ?? propertyInfo.Name;
    ...
    {
        object[] passedArray = new object[collection.Length];
    ...
    public void AddRequestOptions(IEnumerable<IRequestOption> options)
    {
        ...
        foreach (IRequestOption option in options)
        {
            if (option != null)
            {
    ...
    /// </summary>
    public T? GetRequestOption<T>() => _requestOptions.TryGetValue(typeof(T).FullName!, out IRequestOption? requestOption) ? (T)requestOption : default;
    ...
    public void SetResponseHandler(IResponseHandler responseHandler)
    {
        ...
        ResponseHandlerOption responseHandlerOption = new()
        {
    ...
    {
        using Activity? activity = _activitySource?.StartActivity(nameof(SetStreamContent));
    ...
    {
        using Activity? activity = _activitySource?.StartActivity(nameof(SetContentFromParsable));
        using ISerializationWriter writer = GetSerializationWriter(requestAdapter, contentType, items);
    ...
    {
        using Activity? activity = _activitySource?.StartActivity(nameof(SetContentFromParsable));
        using ISerializationWriter writer = GetSerializationWriter(requestAdapter, contentType, item);
    ...
    private static void SetRequestType(object? result, Activity? activity)
    {
        ...
        activity.SetTag("requesty.runtime.request.type", result.GetType().FullName);
    }
    ...
    private static ISerializationWriter GetSerializationWriter<T>(IRequestAdapter requestAdapter, string contentType, T item)
    {
        ...
        if (item == null)
        {
            throw new InvalidOperationException($"{nameof(item)} cannot be null");
        }
    ...
    {
        using Activity? activity = _activitySource?.StartActivity(nameof(SetContentFromScalarCollection));
        using ISerializationWriter writer = GetSerializationWriter(requestAdapter, contentType, items);
    ...
    {
        using Activity? activity = _activitySource?.StartActivity(nameof(SetContentFromScalar));
        using ISerializationWriter writer = GetSerializationWriter(requestAdapter, contentType, item);
        SetRequestType(item, activity);
        switch (item)
        {
    ...
    {
        using Activity? activity = _activitySource?.StartActivity(nameof(SetContentFromEnum));
        using ISerializationWriter writer = GetSerializationWriter(requestAdapter, contentType, item);
        SetRequestType(item, activity);
